...
from .helpers import ConvHandler, FlatDActsBatcher, FlatSegBatcher, Logger
...
class DaTrainingHandler:
    ...
    def train_act(self, args:namedtuple):
        ...
        self.act_model = FlatTransModel(args.system, 43)
        self.act_batcher = FlatDActsBatcher(args.mode_arg, args.max_len)

        train = ConvHandler(path=args.train_path, label_path=args.label_path, 
                            system=args.system, punct=args.punct, lim=args.lim,
                            action=args.action, hes=args.hes)
        
        if args.dev_path:
            dev = ConvHandler(path=args.dev_path, label_path=args.label_path, 
                              system=args.system, punct=args.punct, lim=args.lim, 
                              action=args.action, hes=args.hes)
        ...
        optimizer = torch.optim.Adam(self.act_model.parameters(), lr=args.lr)
        if args.sched:
            SGD_steps = (len(train_data)*self.epochs)/args.bsz
        ...
        for epoch in range(args.epochs):
            ...
            for k, batch in enumerate(self.act_batcher.batches(train.data, args.bsz, shuf=True), start=1):
                ...
                if k%args.print_len == 0:
                    self.L.log(f'  {epoch:<3}      {k:<5}     {logger[0]/args.print_len:.3f}      {logger[1]/logger[2]:.3f}')
                    logger = np.zeros(3)
            
            if args.dev_path:
                preds, labels, loss = self.act_eval(dev.data)
    ...
    @no_grad
    def act_eval(self, eval_data:list)->Tuple[list, list, int]:
        """Returns predictions and labels for DA classification"""
 
        self.to(self.device)        
        predicted_probs, labels, loss = [], [], 0
        for k, batch in enumerate(self.act_batcher.batches(eval_data), start=1):
    ...
    def select_data(self, cfg, mode):
        D = ConvHandler(data_src=cfg.data_src, system=cfg.system, class_reduct=cfg.class_red, 
