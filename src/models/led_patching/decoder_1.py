...
class LEDDecoderPatched(LEDPreTrainedModel):
    ...
    def forward(
        ...
        return_dict=None,
        pos_encoding=None # 3 added code line al826
    ):
        ...
        positions_2 = pos_encoding.to(positions.device) # 3 added code line al826

        #hidden_states = inputs_embeds + positions      
        hidden_states = inputs_embeds + positions + positions_2 #  added code line al826
...
class LEDModelPatched(LEDPreTrainedModel):
    def __init__(self, config: LEDConfig):
        ...
        self.decoder = LEDDecoderPatched(config, self.shared)

        print(self.decoder.__class__)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        ####### >> 1 added code block al826
        pos_encoding = self.encoder.embed_positions(input_ids.size()) # added line 1
        zero_pos = pos_encoding[0].tolist()
        
        #get the specific positional encodings related to [CLS] and [SEP]
        output = []
        for k, ids in enumerate(input_ids):
            output.append([zero_pos] + pos_encoding[ids == 2].tolist())
            temp = torch.FloatTensor([zero_pos] + pos_encoding[ids == 2].tolist())
        #output = [bsz, L, 768] where L is dynamic num_turns length
        
        #pad sequences so I can put it in a tensor
        zeros = [0]*len(pos_encoding[0])
        max_len = max([len(i) for i in output])
        padded_pos = [i + [zeros for _ in range(max_len - len(i))] for i in output]
        padded_pos = torch.FloatTensor(padded_pos)   
        #padded_pos = [bsz, L, 768] where L is max num_turns length

        ######## << 1 end of code block al826
